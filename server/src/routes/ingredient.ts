...
router.get('/low-stock', authenticate, authorize('ADMIN', 'STAFF'), requireStaffPermission('INVENTORY_ADJUST'), ingredientController.getLowStock);
router.get('/:id', validate(idParamSchema), ingredientController.findById);
router.get('/:id/transactions', authenticate, authorize('ADMIN', 'STAFF'), requireStaffPermission('INVENTORY_ADJUST'), validate(idParamSchema), ingredientController.getTransactions);
...
router.post('/:id/adjust-stock', authenticate, authorize('ADMIN', 'STAFF'), requireStaffPermission('INVENTORY_ADJUST'), validate(idParamSchema), validate(stockAdjustmentSchema), ingredientController.adjustStock);
