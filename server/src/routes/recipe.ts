...
import { idParamSchema, productIdParamSchema } from '../validators/auth';

const router = Router();

router.get('/', authenticate, authorize('ADMIN', 'STAFF'), validate(recipeQuerySchema), recipeController.findAll);
router.get('/product/:productId', validate(productIdParamSchema), recipeController.findByProductId);
router.get('/:id', authenticate, authorize('ADMIN', 'STAFF'), validate(idParamSchema), recipeController.findById);
router.get('/:productId/check-stock', validate(productIdParamSchema), recipeController.checkStock);
