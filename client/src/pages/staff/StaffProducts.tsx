    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#7C4EEE]/10 text-[#7C4EEE] border border-[#7C4EEE]/20">
              {t('staffProducts.countBadge', { count: products.length })}
            </span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            {t('staffProducts.subtitle')}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder={t('staffProducts.searchPlaceholder')}
            value={search}
        ...
        >
          <option value="">{t('staffProducts.allCategories', { count: categories.length })}</option>
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center rounded-2xl bg-card border border-dashed border-border/80 text-muted-foreground text-xs space-y-2">
          ...
          <p className="font-semibold text-foreground">{t('staffProducts.emptyTitle')}</p>
          <p className="text-[11px]">{t('staffProducts.emptyDesc')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    <span className="text-[10px] uppercase font-bold text-muted-foreground block truncate tracking-wider">
                      {product.category?.name || t('common.general')}
                    </span>
